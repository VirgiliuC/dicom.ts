...
describe("dicom.ts", () => {
	it("Renders with: RLE decode and 'contrastify' greyscale render", async () => {
		...
		const image = dicomjs.parseDcmObj(new Uint8Array(data).buffer); // use array buffer
	...
	it("Renders with: RLE decode and greyscale window render", async () => {
		...
		const image = dicomjs.parseDcmObj(dataView); // use DataView
		// logImageTags(image);
		const canvas = createCanvas(512, 512);
		const renderer = new dicomjs.Renderer(canvas);
		await renderer.render(image, 0);
	...
	it("Renders with: RLE decode and RGB render", async () => {
		...
		const dataView = new DataView(new Uint8Array(data).buffer);
		const image = dicomjs.parseDcmObj(dataView);
		const canvas = createCanvas(512, 512);
		const renderer = new dicomjs.Renderer(canvas);
		await renderer.render(image, 0);
		// logImageTags(image);
